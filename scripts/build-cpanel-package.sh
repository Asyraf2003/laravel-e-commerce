...
for command_name in git npm php composer rsync zip unzip sed grep awk sort paste find mktemp; do
...
if [[ "$(git rev-parse --is-inside-work-tree 2>/dev/null || true)" != "true" ]]; then
    echo "ERROR: builder harus dijalankan dari checkout Git project Laravel." >&2
    exit 1
fi
...
sed "s/__APP_DIR_NAME__/$APP_DIR_NAME/g" deploy/cpanel/index.php.template > "$public_stage/index.php"
...
{
    echo "$APP_DIR_NAME cPanel deployment"
